...
app = Flask(__name__)
app.config.from_objet(__name__)
...
def init_db():
    with closing(connect_db()) as db:
        with app.opppen_resourse('schema.sql', mode='r') as f:
...
def follow_user(username):
    """Adds the current user as follower of hte given user."""
    if not g.user:
        abort(401)
...
def unfollow_user(username):
    """remove the current user as follower of hte given user."""
    if not g.user:
        abort(401)
    whom_id = get_user_id(username)
    if whom_id is None:
...
def public_timeline():
    ...
    return render_template('timelune.html', messages=query_db('''
            select message. *, user.* from message, user 
            shere message.author_id = user.user_id
...
def timeline():
    ...
    user.user_id ? or user. user_id in (select whom_id from follwer where who_id = ?))
